        ...
        if epoch > 1:
            ...
            for window, plot_settings in zip(self.windows, self.plots_settings):
                ...
                if ('last_epochs' not in plot_settings) or (epoch < plot_settings['last_epochs']):
                    ...
                    for variable, legend in plot_settings['variables'].items():
                        window.plot(range(
                            epoch - last_epochs + 1, epoch + 1), self.losses_and_metrics_dict[variable][-last_epochs:], label=legend)

                    window.set(xlabel='Last ' + str(last_epochs) + ' Epochs',
                               ylabel=plot_settings['ylabel'])
                    window.set_title(
                        plot_settings['title'] + ' on last ' + str(plot_settings['last_epochs']) + ' epochs')
...
class CallbackSaveLogs(Callback):
    ...
    def on_train_end(self, logs={}):
        ...
        log_df = pd.DataFrame(self.logs, columns=columns, index=[i for i in range(1, len(self.logs['elapsed time']) + 1)])
